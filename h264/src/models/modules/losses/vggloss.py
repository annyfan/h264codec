"""A VGG-based perceptual loss function for PyTorch.
   code from: https://github.com/crowsonkb/vgg_loss
"""
...
class VGGLoss(nn.Module):
    ...
    def __init__(self, model='vgg16', layer=8, shift=0, reduction='mean', device=torch.device('cuda:0')):
        super().__init__()
        self.shift = shift
        self.reduction = reduction
        self.normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                              std=[0.229, 0.224, 0.225])
        self.model = self.models[model](weights='IMAGENET1K_FEATURES').features[:layer+1]
    ...
    #Pixel values should be normalized to the range 0–1
