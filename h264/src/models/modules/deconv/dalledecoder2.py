...
class Conv2d(nn.Module):
	...
	def __attrs_post_init__(self) -> None:
		...
		nn.init.normal_(w, mean=0, std=1 / math.sqrt(self.n_in * self.kw ** 2))
...
class Decoder(nn.Module):
	...
	device:              torch.device = attr.ib(default=torch.device('cuda:0'))
	requires_grad:       bool         = attr.ib(default=True)
	...
	def __attrs_post_init__(self) -> None:
		...
		make_blk   = partial(DecoderBlock, n_layers=n_layers, device=self.device,
				requires_grad=self.requires_grad, use_float16=self.use_float16)
		...
		self.blocks = nn.Sequential(OrderedDict([
			...
			('group_2', nn.Sequential(OrderedDict([
				*[(f'block_{i + 1}', make_blk(8 * self.n_hid, 8 * self.n_hid)) for i in blk_range],
				('upsample', nn.Upsample(scale_factor=2, mode='nearest')),
			]))),
			...
			('group_4', nn.Sequential(OrderedDict([
				*[(f'block_{i + 1}', make_blk( 4 * self.n_hid, 4 * self.n_hid)) for i in blk_range],
				('upsample', nn.Upsample(scale_factor=2, mode='nearest')),
			]))),
